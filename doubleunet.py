...
class build_doubleunet(nn.Module):
    def __init__(self):
        ...
        self.a1 = ASPP(512, 64)
        self.d1 = decoder1()
        self.y1 = nn.Conv2d(32, 1, kernel_size=1, padding=0)  # 输出1个通道
        self.sigmoid = nn.Sigmoid()

        self.e2 = encoder2()
        self.a2 = ASPP(256, 64)
        self.d2 = decoder2()
        self.y2 = nn.Conv2d(32, 1, kernel_size=1, padding=0)  # 输出1个通道
    ...
    def forward(self, x):
        ...
        x = self.d1(x, skip1)
        y1 = self.y1(x)
        y1 = self.sigmoid(y1)  # 添加Sigmoid激活函数

        input_x = x0 * self.sigmoid(y1)  # 可以根据需要将y1加入调整输入
        x, skip2 = self.e2(input_x)
        x = self.a2(x)
        x = self.d2(x, skip1, skip2)
        y2 = self.y2(x)
        y2 = self.sigmoid(y2)  # 添加Sigmoid激活函数
